...
integrationDescribe('Schema shape (integration)', () => {
  ...
  const TENANT_TABLES = [
    ...
    'audit_log', 'consent_record', 'alert',
    'era_835_record', 'denial_event', 'abn_record',
    'rule_dispute',
    'client_doc_upload', 'redaction_event', 'webhook_subscription', 'webhook_delivery',
    'cms_0057_pa_response',
    // Phase 11–24
    'subscription', 'billing_event',
    'invite_token',
    'email_send',
    'idempotency_record',
    'synthesis_cache',
  ...
  const REFERENCE_TABLES = [
    ...
    'cob_rule', 'documentation_requirement', 'mhpaea_parity_pair',
    'dme_master_list', 'wc_state_fee_schedule', 'ihs_encounter_rate',
    'feature_flag', 'asc_payment_indicator', 'ub04_bill_type', 'revenue_code_product_line',
    'extraction_candidate', 'extraction_decision', 'attestation_reverification',
  ];
  ...
  it.each(NON_RLS_PLATFORM_TABLES)('%s has RLS DISABLED (platform-global by design)', async (tbl) => {
  ...
  it('seed loaded states + product_lines + POS', async () => {
    const states = await sql<{ count: number }>`SELECT count(*)::int AS count FROM state`.execute(ctx.db);
    const productLines = await sql<{ count: number }>`SELECT count(*)::int AS count FROM product_line`.execute(ctx.db);
    const posCodes = await sql<{ count: number }>`SELECT count(*)::int AS count FROM pos`.execute(ctx.db);
