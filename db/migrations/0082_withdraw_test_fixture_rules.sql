...
SELECT DISTINCT ON (fr.id) fr.id AS fixture_rule_id, prev.id AS revive_id
  ...
  JOIN payer_rule prev
   ...
   AND prev.product_line IS NOT DISTINCT FROM fr.product_line
   AND prev.id <> fr.id
   AND prev.expiration_date IS NOT NULL
   AND prev.expiration_date >= fr.effective_date
   AND (prev.created_by LIKE 'extract:%' OR prev.created_by LIKE 'crawler:%'
        OR prev.created_by LIKE 'seed:%')
...
BEGIN
  ...
  -- Every key the fixture touched must still answer with something.
  ...
  IF dark IS NOT NULL THEN
    RAISE EXCEPTION 'keys left with no live rule after withdrawing fixtures: %', dark;
  END IF;

  RAISE NOTICE 'fixture rules withdrawn: %, displaced rules revived: %, keys left dark: 0',
    n_fixture, n_revived;
END $$;
